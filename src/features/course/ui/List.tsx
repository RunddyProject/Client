...
const CourseList = ({ onViewModeChange }: CourseListProps) => {
  ...
  return (
    <div className='bg-background flex h-full flex-col px-5 pt-[calc(env(safe-area-inset-top)+52px)]'>
      {/* Search bar */}
      ...
      <div className='no-scrollbar mt-3 flex-1 overflow-y-auto pb-[calc(env(safe-area-inset-bottom)+80px)]'>
        {courses.map((course) => (
          <CourseInfoCard
            key={course.uuid}
            course={course}
