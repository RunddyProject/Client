...
export type EnvType = 'PARK' | 'TRAIL' | 'TRACK' | 'URBAN' | 'BEACH' | 'MOUNTAIN' | 'FOREST' | 'ETC';
export type EnvTypeName = '공원' | '산책로' | '트랙' | '도심' | '해변' | '산' | '숲' | '기타';
...
export const shapeTypeNames = Object.keys(SHAPE_NAME_TO_TYPE) as ShapeTypeName[];
...
export const CoursesApi = {
  getCourses: async (lat: number, lng: number, radius?: number): Promise<CoursesResponse> => {
    ...
    return api.get<CoursesResponse>(`/course?${params.toString()}`, { requiresAuth: false });
  },
};
