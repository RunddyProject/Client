...
vi.mock('@/features/strava/hooks/useStravaConnect', () => ({
  useStravaConnect: () => ({ connect: mockConnect, isConnecting: false, error: null })
}));
...
function mockActivitiesSuccess(activities = [makeActivity(1), makeActivity(2)]) {
...
describe('StravaActivitiesPage', () => {
  ...
  it('GPX coursePointList가 비어있으면 에러 토스트', async () => {
    const user = userEvent.setup();
    mockActivitiesSuccess([makeActivity(1)]);
    mockGetActivityGpx.mockResolvedValue({ ...mockGpxResponse, coursePointList: [] });
  ...
  it('isError + 401 시 재연결 UI 표시', () => {
    ...
    expect(screen.getByRole('button', { name: 'Strava 연결하기' })).toBeInTheDocument();
  });
